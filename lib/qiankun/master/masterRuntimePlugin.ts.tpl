...
export async function render(oldRender: typeof noop) {
  ...
  let masterApps = masterOptions.apps || [];

  // 处理xingixngID
  const xingixngApps = masterApps.filter(({ xingxingId }) => xingxingId);
  if (xingixngApps.length) {
    try {
      const promiseApps =  xingixngApps.map(async (app) => await window.fetch('https://test.com/fetch', { 
        method: 'POST',
        mode: 'cors',
        credentials: 'include',
        headers: {
        ...
        body: JSON.stringify({ xingxingId: app.xingxingId })
        }));
      const res = await promise.all(promiseApps) || [];
      masterApps = masterApps.filter(({ xingxingId }) => !xingxingId).concat(res);
    } catch(e) {
      console.error(e);
      masterApps = masterApps.filter(({ xingxingId }) => !xingxingId);
    }
  }
