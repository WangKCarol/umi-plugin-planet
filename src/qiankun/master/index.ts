...
export default function (api: IApi) {
  ...
  api.onGenerateFiles(() => {
    ...
    api.writeTmpFile({
      ...
      content: readFileSync(join(__dirname, '../../../src/qiankun/common.ts'), 'utf-8'),
    });
    api.writeTmpFile({
      path: 'plugin-qiankun/constants.ts',
      content: readFileSync(join(__dirname, '../../../src/qiankun/constants.ts'), 'utf-8'),
    });
    api.writeTmpFile({
      path: 'plugin-qiankun/types.ts',
      content: readFileSync(join(__dirname, '../../../src/qiankun/types.ts'), 'utf-8'),
    });
